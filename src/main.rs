...
fn main() {
    ...
    let matches = search_file(file, &pattern).unwrap();
    if matches.len() > 0 {
        for m in matches {
            println!("{}", m);
        }
...
fn search_file<R : Read>(file: R, pattern_str: &str) -> Result<Vec<String>> {
...
fn find_line_match<R : Read>(buffer: &mut FileBuffer<R>, pattern: &pattern::Pattern) -> Result<String> {
    ...
    fn read_back(&self, distance: usize) -> Option<u8> {
        if self.offset < distance {
            if self.high_water_mark > 0 {
                if distance - self.offset > self.high_water_mark - self.next_available_index {
